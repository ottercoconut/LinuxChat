...
- 服务端数据库互斥、事务、级联外键、唯一约束、预处理语句和会话授权契约
- P2 安全契约，包括协议分隔符拒绝、密码哈希、SQL 注入防回退和客户端 `snprintf` 构造
...
为兼容现有客户端，`ADDFRIEND`、`FRIENDS`、`MESSAGES` 和 `SEND` 仍保留 user_id/sender_id 字段；服务端实际执行时只信任登录会话中的用户 ID。用户名、密码、昵称不能包含 `,`、`:`、`;` 或换行，消息内容不能包含 `:`、`;` 或换行。
...
当前版本是课程设计/学习用途实现，已覆盖主要功能链路。issue #3 已修复登录后聊天链路的 P0 阻断问题，包括客户端接收线程启动时机、聊天页切换、服务端在线用户状态同步和实时消息昵称获取。issue #5 已修复 P1 稳定性和数据正确性问题，包括历史消息时间戳解析、历史响应缓冲区、socket 失败判断和全局 MySQL 连接并发访问保护。P2 安全加固已完成 SQL 预处理语句、密码哈希、协议分隔符校验、客户端有界格式化，以及好友/消息操作基于登录会话授权。
