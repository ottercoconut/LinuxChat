...
| 模块名称 | 功能描述 | 所属层次 |
|---------|---------|---------|
| 用户注册模块 | 用户通过用户名、密码和用户名进行注册 | 客户端+服务器 |
...
- 用户可以通过客户端注册页面创建新账号，需要填写用户名、密码和用户名
- 用户可以通过登录页面验证身份并进入系统
- 用户名具有唯一性约束，避免重复注册
- 登录状态通过用户ID进行管理
...
4. **验证结果**：
   - 如果匹配成功，返回登录成功消息，包含用户ID、用户名和用户名
   - 如果匹配失败，返回登录失败消息
...
```c
    ...
    char username[50], password[50], username[50];
    int user_id;

    if (sscanf(buffer + 6, "%49[^,],%49[^\n]", username, password) == 2 &&
        login_user(username, password, username, &user_id) == 0) {
        client->user_id = user_id;
        strncpy(client->username, username, sizeof(client->username) - 1);
        client->username[sizeof(client->username) - 1] = '\0';
        snprintf(response, sizeof(response),
                 "LOGIN_SUCCESS:%d:%s:%s", user_id, username, username);
    } else {
...
```c
    ...
    snprintf(response, sizeof(response),
             "NEW_MESSAGE:%d:%s:%s", sender_id, client->username, content);
...
| username | VARCHAR(50) | NOT NULL | 用户用户名 |
...
```c
int register_user(const char *username, const char *password, const char *username) {
    const char *statement =
        "INSERT INTO users (username, password, username) VALUES (?, SHA2(?, 256), ?)";
    ...
    bind_string_param(&params[2], username, &lengths[2]);
...
| 命令 | 参数 | 说明 |
|------|------|------|
| REGISTER | username,password,username | 用户注册 |
...
| 测试用例 | 输入 | 预期结果 | 实际结果 |
|---------|------|---------|---------|
| 正常注册 | 用户名：test1，密码：123456，用户名：测试用户 | 注册成功，返回user_id | 成功 |
| 用户名重复 | 用户名：test1，密码：123456，用户名：测试用户2 | 返回"REGISTER_FAILED" | 成功 |
...
| 在线会话同步 | 登录后 `clients` 数组同步用户ID、用户名和用户名 | 成功 |
...
void update_client_session(int sockfd, int user_id, const char *username, const char *username) {
    pthread_mutex_lock(&clients_mutex);
    for (int i = 0; i < client_count; i++) {
...
int login_user(const char *username, const char *password, char *username, int *user_id) {
    const char *statement =
    ...
    results[1].buffer = username_buffer;
    results[1].buffer_length = sizeof(username_buffer) - 1;
    ...
    if (fetch_status == 0 || fetch_status == MYSQL_DATA_TRUNCATED) {
        size_t copy_length = username_length < 49 ? username_length : 49;
        username_buffer[copy_length] = '\0';
        if (!is_protocol_safe_text(username_buffer, 0)) {
            goto cleanup;
        }

        *user_id = found_user_id;
        strncpy(username, username_buffer, 49);
        username[49] = '\0';
        status = 0;
    }
...
int get_group_members(int requester_id, int group_id, char *result, size_t result_size) {
    ...
    while (mysql_stmt_fetch(stmt) == 0) {
        ...
        if (append_protocol_record(result, result_size, id_text, username, username) != 0) {
...
void *handle_client(void *arg) {
    ...
    while (1) {
        ...
        if (strncmp(buffer, "REGISTER:", 9) == 0) {
            char username[50], password[50], username[50];
            if (sscanf(buffer + 9, "%49[^,],%49[^,],%49[^\n]", username, password, username) == 3) {
                int user_id = register_user(username, password, username);
                if (user_id > 0) {
            ...
            if (sscanf(buffer + 6, "%49[^,],%49[^\n]", username, password) == 2 &&
                ...
                strncpy(client->username, username, sizeof(client->username) - 1);
                client->username[sizeof(client->username) - 1] = '\0';
                update_client_session(client->sockfd, user_id, username, username);
                snprintf(response, sizeof(response), "LOGIN_SUCCESS:%d:%s:%s", user_id, username, username);
                login_success = 1;
            } else {
                snprintf(response, sizeof(response), "LOGIN_FAILED");
            }
            ...
            snprintf(response, sizeof(response), "NEW_MESSAGE:%d:%s:%s", sender_id, client->username, content);
            send_to_user(receiver_id, response);
            send(client->sockfd, response, strlen(response), 0);
        }
            ...
            snprintf(response, sizeof(response), "NEW_GROUP_MESSAGE:%d:%d:%s:%s",
                     group_id, sender_id, client->username, content);
...
void on_register_clicked(GtkButton *button, gpointer user_data) {
    ...
    if (strlen(username) == 0 || strlen(password) == 0 || strlen(username) == 0) {
        gtk_label_set_text(GTK_LABEL(status_label), "请填写完整信息");
        return;
    }
    if (strlen(username) >= 50 || strlen(password) >= 50 || strlen(username) >= 50) {
        gtk_label_set_text(GTK_LABEL(status_label), "用户名、密码和用户名不能超过49字节");
        return;
    }
    ...
    if (snprintf(msg, sizeof(msg), "REGISTER:%s,%s,%s", username, password, username) >= (int)sizeof(msg)) {
...
void on_login_clicked(GtkButton *button, gpointer user_data) {
    ...
    if (strncmp(buffer, "LOGIN_SUCCESS:", 14) == 0 &&
        sscanf(buffer + 14, "%d:%49[^:]:%49[^\n]", &current_user_id, current_username, current_username) == 3) {
...
void on_friend_selected(GtkTreeView *tree_view, gpointer user_data) {
    ...
    if (gtk_tree_selection_get_selected(selection, &model, &iter)) {
        ...
        gtk_tree_model_get(model, &iter, 0, &friend_id, 1, &username, -1);
        ...
        char title[100];
        snprintf(title, sizeof(title), "与 %s 聊天", username);
...
gboolean parse_friends_list(gpointer data) {
    ...
    char friend_id[10], username[50], username[50];

    while (sscanf(ptr, "%9[^:]:%49[^:]:%49[^;];", friend_id, username, username) == 3) {
        GtkTreeIter iter;
        gtk_list_store_append(store, &iter);
        gtk_list_store_set(store, &iter, 0, atoi(friend_id), 1, username, -1);

        ptr += strlen(friend_id) + strlen(username) + strlen(username) + 3;
    }
...
gboolean parse_group_members_list(gpointer data) {
    ...
    char member_id[10], username[50], username[50];

    while (sscanf(ptr, "%9[^:]:%49[^:]:%49[^;];", member_id, username, username) == 3) {
        GtkTreeIter iter;
        char display[120];
        snprintf(display, sizeof(display), "%s (%s)", username, member_id);
        gtk_list_store_append(store, &iter);
        gtk_list_store_set(store, &iter, 0, atoi(member_id), 1, display, -1);

        ptr += strlen(member_id) + strlen(username) + strlen(username) + 3;
    }
...
gboolean parse_new_message(gpointer data) {
    char *buffer = (char *)data;
    int sender_id;
    char sender_username[50], content[BUFFER_SIZE];

    if (sscanf(buffer + 12, "%d:%49[^:]:%1023[^\n]", &sender_id, sender_username, content) == 3) {
        if (sender_id == selected_friend_id || sender_id == current_user_id) {
            ...
            snprintf(msg, sizeof(msg), "%s [%s]: %s\n", sender_username, time_str, content);
            gtk_text_buffer_insert(text_buffer, &iter, msg, -1);
...
gboolean parse_new_group_message(gpointer data) {
    ...
    if (sscanf(buffer + 18, "%d:%d:%49[^:]:%1023[^\n]",
               &group_id, &sender_id, sender_username, content) == 4) {
