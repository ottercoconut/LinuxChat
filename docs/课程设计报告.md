...
我们实现的是一个基于 Linux 平台的网络即时通信工具。服务端使用 C 语言、Socket 和 MySQL，客户端使用 GTK3 做图形界面。用户启动客户端后，可以完成注册、登录、添加好友、一对一聊天、群聊、屏蔽用户和查看离线消息提示等操作。
...
| 屏蔽模块 | 屏蔽或解除屏蔽用户，私聊发送前检查屏蔽关系 | 客户端+服务器 |
...
- 用户可以屏蔽好友，也可以解除屏蔽
- 私聊发送前会检查双方是否存在屏蔽关系，存在屏蔽关系时消息不会写入数据库
...
- **用户信息区域**：显示当前登录用户信息，提供添加好友、创建群聊、添加群成员、屏蔽好友和解除屏蔽按钮
...
| 测试用例 | 操作 | 预期结果 | 实际结果 |
|---------|------|---------|---------|
| 屏蔽用户 | 用户2屏蔽用户1后，用户1尝试私聊用户2 | 服务端拒绝发送 | 成功 |
| 解除屏蔽 | 用户2解除对用户1的屏蔽 | 私聊发送资格恢复 | 成功 |
...
到目前为止，项目已经能够跑通主要聊天流程。客户端和服务端可以正常启动，用户可以注册、登录、添加好友，进行一对一消息收发，也可以创建群聊、查看群成员、发送群消息、屏蔽用户和查看离线消息提示。我们实际完成的内容包括：
