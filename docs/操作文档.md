...
3. 协议记录拼接的边界和截断行为，以及 TCP 文本协议的粘包/拆包处理。
4. 客户端 GTK idle 回调、页面切换和限长解析检查。
5. 服务端数据库互斥、事务、级联外键、唯一约束、预处理语句和响应构造检查。
6. 安全加固检查，包括协议分隔符拒绝、会话授权、密码哈希和客户端 `snprintf` 构造。
7. 用户名解析路径，包括 username 查找、找不到用户、自我添加拒绝和群成员权限仍以登录会话用户为准。
8. `init.sql` 的关键约束、密码哈希和可重复导入特性。
...
1. 点击"创建群聊"，输入群名称；创建者会自动成为群成员。
2. 在群聊列表中选择群聊后，可以查看群历史和成员列表。
3. 选择群聊后点击"添加群成员"，输入成员用户名即可邀请成员加入。
4. 选择群聊后输入文本并发送，即可向在线群成员实时推送消息，离线成员重新登录后会看到未读提示，也可以通过群历史确认消息。
5. 点击"拉黑好友"并输入用户名后，双方私聊发送会被服务端拒绝；需要恢复时点击"解除拉黑"并输入用户名。
6. 好友登录和退出时，在线好友会收到上线或离线提示。
...
客户端和服务器之间使用文本协议。客户端发送的每条命令以换行符 `\n` 作为帧边界，服务端维护连接级接收缓冲区并按完整行解析，因此可以处理 TCP 粘包和拆包。命令本体格式如下：
...
普通用户交互优先使用 `username`，服务端通过预处理语句将 `users.username` 解析为 `users.id`。`users.id` 是 8 位随机数字，和用户名一起作为用户身份标识。为兼容现有客户端，部分请求仍携带 user_id/sender_id；服务端实际执行时只信任登录会话中的用户 ID。用户名、密码和群名称不能包含 `,`、`:`、`;` 或换行，消息内容不能包含 `:`、`;` 或换行。
...
| REGISTER_FAILED_DUPLICATE_USERNAME | 注册失败，用户名已存在 |
| REGISTER_FAILED_INVALID_INPUT | 注册失败，输入包含非法分隔符或超长 |
| REGISTER_FAILED_SERVER_ERROR / REGISTER_FAILED | 注册失败，服务端错误或兼容旧失败响应 |
| LOGIN_SUCCESS:user_id:username | 登录成功 |
| LOGIN_FAILED | 登录失败 |
| ADDFRIEND_SUCCESS | 添加好友成功 |
| ADDFRIEND_FAILED | 添加好友失败 |
| FRIENDS_LIST:id:username:username;... | 好友列表 |
| MESSAGES_LIST:content:timestamp:username;... | 消息列表 |
| NEW_MESSAGE:sender_id:username:timestamp:content | 新消息通知 |
| GROUPS_LIST:id:name:owner_username;... | 群聊列表 |
| GROUP_MEMBERS_LIST:id:username:username;... | 群成员列表 |
| GROUP_MESSAGES_LIST:content:timestamp:username;... | 群消息列表 |
| NEW_GROUP_MESSAGE:group_id:sender_id:username:timestamp:content | 新群消息通知 |
| CREATE_GROUP_SUCCESS:group_id / CREATE_GROUP_FAILED | 创建群聊结果 |
| ADD_GROUP_MEMBER_SUCCESS | 添加群成员成功 |
| ADD_GROUP_MEMBER_FAILED_ALREADY_MEMBER | 添加群成员失败，用户已在群内 |
| ADD_GROUP_MEMBER_FAILED_BLOCKED | 添加群成员失败，双方存在拉黑关系 |
| ADD_GROUP_MEMBER_FAILED | 添加群成员失败 |
| SEND_FAILED / SEND_FAILED_BLOCKED | 私聊发送失败或被拉黑关系阻止 |
| SEND_GROUP_FAILED | 群消息发送失败 |
| BLOCK_USER_SUCCESS / BLOCK_USER_FAILED | 拉黑结果 |
| UNBLOCK_USER_SUCCESS / UNBLOCK_USER_FAILED_NOT_BLOCKED / UNBLOCK_USER_FAILED_BLOCKED_BY_TARGET / UNBLOCK_USER_FAILED | 解除拉黑结果 |
| OFFLINE_MESSAGES_LIST:type:id:count;... | 未读消息摘要 |
| FRIEND_ONLINE:user_id:username | 好友上线通知 |
| FRIEND_OFFLINE:user_id:username | 好友离线通知 |
...
| username | VARCHAR(50) | UNIQUE NOT NULL | 公开唯一用户名 |
...
如果编译后运行时报 `Library not loaded: @rpath/libmysqlclient.24.dylib`、`Library not loaded: libssl.3.dylib` 或 `Library not loaded: libcrypto.3.dylib`，说明 MySQL 相关动态库路径没有被正确记录。可以用下面的命令修复当前目录下的 `server`：
...
**原因**：用户名已存在、用户名/密码不完整，或输入包含协议不支持的分隔符

**解决方案**：
- 检查用户名是否已被注册
- 确保填写用户名和密码
- 确保用户名和密码不包含逗号、冒号、分号或换行
...
| get_messages() | 获取消息记录 |
| save_message() | 保存消息 |
| block_user() / unblock_user() | 拉黑或解除拉黑用户 |
| create_group() | 创建群聊并写入创建者成员关系 |
...
| on_block_user_clicked() / on_unblock_user_clicked() | 拉黑或解除拉黑操作 |
...
4. 服务端 `SEND` 分支使用登录会话中的用户名，不再通过空密码重新调用登录函数。
...
7. 客户端请求使用换行符作为协议帧边界，服务端按完整行解析，避免 TCP 粘包/拆包导致命令合并或截断。
