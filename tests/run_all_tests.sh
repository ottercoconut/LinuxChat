...
assert_contains "$CLIENT_SOURCE" "clear_tree_view_selection(groups_list)"
assert_contains "$CLIENT_SOURCE" "clear_tree_view_selection(friends_list)"
assert_contains "$CLIENT_SOURCE" "gdk_threads_add_idle(show_message_dialog, g_strdup(\"你与该好友存在拉黑关系，无法发送消息\"))"
assert_contains "$CLIENT_SOURCE" "gdk_threads_add_idle(show_message_dialog, g_strdup(\"该用户已经是群成员，无需重复添加\"))"
assert_contains "$CLIENT_SOURCE" "gdk_threads_add_idle(show_message_dialog, g_strdup(\"你与该用户存在拉黑关系，无法添加为群成员\"))"
assert_contains "$CLIENT_SOURCE" "gdk_threads_add_idle(show_message_dialog, g_strdup(\"已解除拉黑\"))"
assert_contains "$CLIENT_SOURCE" "gdk_threads_add_idle(show_message_dialog, g_strdup(\"对方已将你拉黑，无法由你解除\"))"
...
assert_contains "$CLIENT_SOURCE" "REGISTER:%s,%s"
assert_contains "$CLIENT_SOURCE" "snprintf(friend_label, sizeof(friend_label), \"%s (%s)\", username, friend_id)"
assert_contains "$CLIENT_SOURCE" "%1023[^:]:%49[^:]:%49[^;];"
assert_contains "$CLIENT_SOURCE" "%d:%49[^:]:%49[^:]:%1023[^\\n]"
assert_contains "$CLIENT_SOURCE" "%d:%49[^:]:%1023[^\\n]"
assert_contains "$CLIENT_SOURCE" "%d:%d:%49[^:]:%49[^:]:%1023[^\\n]"
assert_contains "$CLIENT_SOURCE" "is_client_protocol_safe"
assert_contains "$CLIENT_SOURCE" "%d:%49[^\\n]"
...
assert_not_contains "$CLIENT_SOURCE" "初始成员ID"
assert_not_contains "$CLIENT_SOURCE" "群名称或成员ID格式不正确"
...
assert_contains "$SERVER_SOURCE" "notify_group_members_changed(group_id, client->user_id)"
assert_contains "$SERVER_SOURCE" "if ((server_fd = socket(AF_INET, SOCK_STREAM, 0)) < 0)"
assert_contains "$SERVER_SOURCE" "10000000 + rand() % 90000000"
assert_contains "$SERVER_SOURCE" "generate_unique_user_id_locked"
...
assert_contains "$SERVER_SOURCE" "ADD_GROUP_MEMBER_FAILED_ALREADY_MEMBER"
assert_contains "$SERVER_SOURCE" "ADD_GROUP_MEMBER_FAILED_BLOCKED"
assert_contains "$SERVER_SOURCE" "UNBLOCK_USER_FAILED_NOT_BLOCKED"
assert_contains "$SERVER_SOURCE" "UNBLOCK_USER_FAILED_BLOCKED_BY_TARGET"
assert_contains "$SERVER_SOURCE" "GROUP_MEMBER_ADD_ALREADY_MEMBER"
assert_contains "$SERVER_SOURCE" "GROUP_MEMBER_ADD_BLOCKED"
assert_contains "$SERVER_SOURCE" "UNBLOCK_USER_BLOCKED_BY_TARGET"
assert_contains "$SERVER_SOURCE" "add_group_member(client->user_id, group_id, new_member_id)"
...
assert_contains "$SERVER_SOURCE" "SEND_FAILED_BLOCKED"
assert_contains "$SERVER_SOURCE" "has_block_between(sender_id, receiver_id)"
...
assert_contains "$INIT_SQL" "DROP TABLE IF EXISTS users"
assert_contains "$INIT_SQL" "id INT PRIMARY KEY"
