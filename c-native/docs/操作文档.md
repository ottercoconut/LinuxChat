...
issue #5 修复了聊天链路中的 P1 稳定性和数据正确性问题。项目根目录提供了 `tests/run_p1_tests.sh`，该脚本会先运行 P0 回归测试，再运行 P1 专项测试。
...
1. P0 聊天主链路修复没有回退。
2. 服务端全局 MySQL 连接通过互斥锁串行访问，避免多个客户端线程裸并发调用同一连接。
3. 历史消息时间戳格式不再包含 `:`，避免客户端按分隔符解析时错位。
4. 好友列表和历史消息列表使用带容量的拼接函数，结果过长时会截断而不是越界写入。
5. `MESSAGES_LIST` 不再把大消息缓冲区写入 1KB 的 `response`。
6. 服务端 socket 创建失败判断使用 `< 0`，能正确捕获失败返回值。
7. 服务端响应构造和命令解析减少无界缓冲区写入风险。
...
issue #5 已修复聊天链路中的 P1 稳定性和数据正确性问题：
...
1. SQL 仍使用字符串拼接，存在注入风险，后续应改为 MySQL 预处理语句。
2. 文本协议仍依赖 `:`, `,`, `;` 分隔，消息内容包含这些字符时可能解析错误。
