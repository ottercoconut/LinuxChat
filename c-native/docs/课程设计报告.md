...
| 服务端契约 | 数据库互斥、事务、唯一约束、级联外键、预处理语句和有界响应构造存在 | 成功 |
| P2安全契约 | 协议分隔符拒绝、会话授权、密码哈希和客户端 `snprintf` 构造存在 | 成功 |
| 初始化脚本契约 | `init.sql` 包含唯一约束、级联外键、密码哈希和 `INSERT IGNORE` | 成功 |
...
### 7.1 服务器代码（server.c 节选）

完整源码见 `c-native/server/server.c`。下面保留与安全加固直接相关的核心片段：
